    ...
    static func applyLUT(image: UIImage, lut: UIImage, intensity: CGFloat) -> UIImage? {
        ...
        for y in 0 ..< imageHeight {
            for x in 0 ..< imageWidth {
                let i = (y * imageWidth + x) * 4 // 문제 x
                let r = CGFloat(imagePixelData[i] / 4)
                let g = CGFloat(imagePixelData[i + 1] / 4)
                let b = CGFloat(imagePixelData[i + 2] / 4)
                
                let floorLutIndex = getLUTIndex(red: Int(floor(r)), green: Int(floor(g)), blue: Int(floor(b)), lutWidth: lutWidth)
                var ceilLutIndex = getLUTIndex(red: Int(ceil(r)), green: Int(ceil(g)), blue: Int(ceil(b)), lutWidth: lutWidth) // 63.5 이상 -> 64이면 터지나? 보간 안하면 되지 않나?
